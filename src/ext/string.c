...
Value cc_function_string_trim(int arg_count, Value* args) {
  ...
  while (found_whitespace) {
    ...
    for (int i = 0; whitespace[i] != '\0'; i++) {
      if (str->chars[left_trimlen] == whitespace[i]) {
  ...
  while (found_whitespace) {
    found_whitespace = false;
    for (int i = 0; whitespace[i] != '\0'; i++) {
      if (str->chars[ str->length - 1 - right_trimlen ] == whitespace[i]) {
        found_whitespace = true;
        right_trimlen++;
        break; // out of the for, into the while
      }
  ...
  if (left_trimlen + right_trimlen == 0) { return args[0]; }

  return OBJ_VAL(copyString(&str->chars[left_trimlen], str->length - (left_trimlen + right_trimlen)));
}
...
Value cc_function_string_trim_left(int arg_count, Value* args) {
  ...
  while (found_whitespace) {
    found_whitespace = false;
    for (int i = 0; whitespace[i] != '\0'; i++) {
      if (str->chars[trimlen] == whitespace[i]) {
        found_whitespace = true;
        trimlen++;
        break; // out of the for, into the while
      }
  ...
  return OBJ_VAL(copyString(&str->chars[trimlen], str->length - trimlen));
}
